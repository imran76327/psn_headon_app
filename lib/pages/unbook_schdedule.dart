...
import '../bloc/unbook/unbook_bloc.dart';
import '../entry/entry_point.dart';
import '../home/home_page.dart';
import '../utils/theme/theme_notifier.dart';

class UnbookSchdedule extends StatefulWidget {
  const UnbookSchdedule(
  ...
  @override
  State<UnbookSchdedule> createState() => UnbookSchdeduleState();
}

class UnbookSchdeduleState extends State<UnbookSchdedule>  with TickerProviderStateMixin{
 ...
 _controllers = List.generate(8, (index) {
  ...
  return BlocProvider(
  create: (context) => UnbookBloc(),
  child: BlocConsumer<UnbookBloc, UnbookState>(
    listener: (context, state) {
        if (state is UnbookSuccess) {
          // Show success dialog
          showDialog(
            context: context,
                ...
                child: AnimatedContainer(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      const SizedBox(height: 10),
                      const Text(
                        'Failed to cancel the Slot: Please Try Again!',
                        style: TextStyle(fontSize: 16, color: Colors.white),
                      ),
                      const SizedBox(height: 20),
                      ElevatedButton(
...
const SizedBox(height: 20),
SlideTransition(
  position: _animations[2], // Work Site animation
  child: Padding(
    padding: const EdgeInsets.symmetric(horizontal: 20.0),
    child: Material(
      borderRadius: BorderRadius.circular(10),
...
const SizedBox(height: 20),
SlideTransition(
  position: _animations[3], // Working Hours animation
  child: Padding(
    padding: const EdgeInsets.symmetric(horizontal: 20.0),
    child: Material(
      borderRadius: BorderRadius.circular(10),
                  ...
                  child: isWithinNotificationTime(now)
                      ? ElevatedButton(
                          ...
                          onPressed: () async {
                            ...
                            bool? confirmed = await showDialog(
                              ...
                              builder: (BuildContext context) {
                                return AlertDialog(
                                  ...
                                  content: const Text("Do you want to unbook the slot?"),
                                  actions: <Widget>[
                                    TextButton(
                                      onPressed: () {
                            ...
                            if (confirmed == true) {
                              BlocProvider.of<UnbookBloc>(context).add(
                                Unbookfetch(
                                  slotId: widget.userData.slotid, // Slot ID
